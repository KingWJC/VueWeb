<template>
  <div class="dict-filter clearfix">
    <div class="dict-filter__project">
      <span>项目:</span>
      <input v-model="searchObj"
        type="text"
        placeholder="请输入" />
    </div>

    <div class="dict-filter__btnbox">
      <span>是否启用:</span>
      <select-dropdown-v2
        v-model="cachedValid"
        :dropDownMaxHeight="210"
        :options="validstate"
        label="ITEM_NAME"
        trackBy="ITEM_CODE"
      >
        <template
          slot="drop"
          slot-scope="dropContent">
          <p class="select-dropdown-v2-options__items-item-text">
            {{ dropContent.options.ITEM_NAME }}
          </p>
        </template>
      </select-dropdown-v2>
    </div>

    <div class="dict-filter__searchbox clearfix">
      <div class="dict-filter__searchbox-searchbar clearfix">
        <a class="dict-filter__basicbtn"
          href="javascript:;"
          @click="search">查询</a>
        <a
          href="javascript:;"
          class="dict-filter__basicbtn dict-filter__searchbox-reset"
          @click="reset">重置</a>
      </div>
    </div>
  </div>
</template>

<script>
  import EventBus from "__shared-libs/js/vue/eventbus";
  import SelectDropdownV2 from "__shared-libs/components/select-dropdown/SelectDropdownV2";

  export default {
    name: "dict-filter",

    components: {
      SelectDropdownV2
    },

    data() {
      return {
        searchObj: "",
        cachedValid: null,
        validstate: [
          {
            ITEM_CODE: "0",
            ITEM_NAME: "停用"
          },
          {
            ITEM_CODE: "1",
            ITEM_NAME: "启用"
          }
        ]
      };
    },

    methods: {
      // 搜索
      search() {
        let searchOption = {};
        searchOption.SearchText = this.searchObj.trim();
        searchOption.IsValid = this.cachedValid
          ? this.cachedValid.ITEM_CODE.trim() : "";
        EventBus.$emit("searchChanged", searchOption);
      },

      // 重置
      reset() {
        this.searchObj = "";
        this.cachedValid = "";
        this.search();
      }
    }
  };
</script>

<style lang="scss">
  @import "~__libs/scss/lib-common-utils";

  .dict-filter {
    @include border-radius(4px);
    @include custom-container(sz(22));

    margin-top: sz(10);
    position: relative;
    border: 1px solid map-get($colors, dialog-cancel-border);
    padding: sz(8) sz(10) sz(6) sz(10);
    background: map-get($colors, pure-white);

    &__project {
      float: left;
      margin-top: sz(3);
      margin-left: sz(11);
      width: sz(280);
      span {
        float: left;
        color: black;
        font-weight: bolder;
        margin-right: sz(10);
        line-height: sz(34);
      }
      input {
        @include border-radius(4px 4px 4px 4px);
        @include placeholder-color(map-get($colors, placeholder-text-color));

        border: 1px solid map-get($colors, dropdown-box-border);
        height: sz(32);
        width: sz(200);
        padding-left: sz(10);
        // float: right;
        color: map-get($colors, placeholder-text-color);

        &:focus {
          border: 1px solid map-get($colors, theme-blue);
        }
      }
    }

    &__btnbox {
      float: left;
      margin-top: sz(2);
      width: sz(300);
      span {
        margin-left: sz(30);
        margin-right: sz(10);
        color: black;
        font-weight: bolder;
        line-height: sz(34);
      }
      .select-dropdown-v2 {
        width: sz(200);
        float: right;

        &__selected {
          font-size: map-get($font-sizes, xs);
          height: sz(32);
          line-height: sz(32);
          border-color: map-get($colors, dialog-input-border);

          &-placeholder {
            height: sz(32);
            line-height: sz(32);
            font-size: map-get($font-sizes, xs);
          }

          &-input,
          &-plain {
            height: sz(32);
            font-size: map-get($font-sizes, xs);
            line-height: sz(32);
          }
        }
      }
    }

    &__searchbox {
      float: right;
      margin-right: sz(10);

      &-searchbar {
        float: left;

        input {
          @include border-radius(4px 4px 4px 4px);
          @include placeholder-color(map-get($colors, placeholder-text-color));

          border: 1px solid map-get($colors, dropdown-box-border);
          height: sz(34);
          width: sz(400);
          padding-left: sz(10);
          float: left;
          color: map-get($colors, placeholder-text-color);

          &:focus {
            border: 1px solid map-get($colors, theme-blue);
          }
        }

        a {
          @include border-radius(4px 4px 4px 4px);

          width: sz(90);
          float: left;
          background: map-get($colors, theme-blue);
          color: map-get($colors, pure-white);
        }
      }

      &-reset {
        margin-left: sz(10);
        float: left;

        #{get-bem-root()} & {
          color: map-get($colors, pure-white);
        }
      }
    }

    &__basicbtn {
      @include border-radius(4px);

      display: inline-block;
      vertical-align: middle;
      height: sz(34);
      line-height: sz(34);
      text-align: center;
    }
  }
</style>
