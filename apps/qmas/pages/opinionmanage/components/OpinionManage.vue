<template>
  <div class="opinion-manage">
    ...
    <div class="opinion-manage__tablecontent">
      <template v-if="!isShowLoading">
        <template v-if="bodyData">
          <div class="opinion-manage__tablecontent-body">
            <opinion-table-body :bodyData="bodyData"
              @refreshData="refreshData"></opinion-table-body>
          </div>
          <paginator
        ...
        <template v-else>
          <qa-no-content title="暂无意见反馈"></qa-no-content>
        </template>
      </template>
    </div>
  </div>
...
<script>
  ...
  import OpinionTableBody from "./OpinionTableBody";
  import OpinionTableFilter from "./OpinionTableFilter";
  ...
  export default {
    ...
    mounted() {
      ...
      EventBus.$on("searchContent", (searchContent) => {
        ...
        this.searchType = searchContent.SearchRoleType;
    ...
    methods: {
      ...
      fetchData() {
        ...
        Axios.post(
          resolveApiRoute("opinion-manage-get"),
          this.getApiPara()
        ).then(
          (res) => {
            this.$hideLoading();
