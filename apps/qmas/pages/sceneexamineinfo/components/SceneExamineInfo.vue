<template>
  <div class="scene-examine-info">
    <scene-examine-table-filter
      :disabled="filterDisabled"
    ></scene-examine-table-filter>

    <div class="scene-examine-info__tablecontent">
      <template v-if="!isShowLoading">
        <template v-if="bodyData">
          <div class="scene-examine-info__tablecontent-body">
            <scene-examine-table-body :bodyData="bodyData"
              @refreshData="refreshData"></scene-examine-table-body>
          </div>
        ...
        <template v-else>
          <qa-no-content title="暂无现场审核数据"></qa-no-content>
        </template>
      </template>
    </div>
  </div>
...
<script>
  ...
  import SceneExamineTableBody from "./SceneExamineTableBody";
  import SceneExamineTableFilter from "./SceneExamineTableFilter";

  export default {
    name: "scene-examine-info",

    components: {
      Paginator,
      SceneExamineTableBody,
      SceneExamineTableFilter,
      QaNoContent
    },
    ...
    methods: {
      ...
      fetchData() {
        ...
        Axios.post(
          resolveApiRoute("scene-examine-list"),
          this.getApiPara()
        ).then(
          (res) => {
            this.$hideLoading();
